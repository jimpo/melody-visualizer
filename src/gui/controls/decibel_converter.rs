...
pub fn new(controller: &Rc<RefCell<DecibelConverterController>>)
	-> Result<impl IsA<gtk::Widget>, Error>
{
	...
	min_level_scale.connect_change_value(
		move |_scale, _, value| on_min_level_change(&controller_clone, value)
	);
	...
	let initial_value = match app_controller.config.spectrum_transforms.get(&controller.id()) {
		...
		Some(_) => return Err(Error::UnexpectedConfigEntry(format!(
			"expected decibel converter transform with id {}", controller.id()
		))),
